...
export const getAllBrokers = async (req: Request, res: Response) => {
  try {
    // 1. Verify your database connection/pool is defined
    const result = await pool.query("SELECT * FROM broker_details ORDER BY created_at DESC");
    
    // 2. Always return result.rows, or an empty array if null
    return res.status(200).json(result.rows || []); 
  } catch (error) {
    console.error("Backend Database Error:", error);
    // 3. Return a 500 status with an empty array to prevent frontend crash
    return res.status(500).json([]); 
  }
};

// Approve Broker
export const approveBroker = async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    await pool.query("UPDATE broker_details SET status = 'Approved' WHERE id = $1", [id]);
...
// Reject Broker
export const rejectBroker = async (req: Request, res: Response) => {
  const { id } = req.params;
  const { reason } = req.body;
  try {
    await pool.query(
      "UPDATE broker_details SET status = 'Rejected', rejection_reason = $1 WHERE id = $2", 
      [reason, id]
    );
    res.status(200).json({ message: "Broker rejected successfully" });
  } catch (error) {
